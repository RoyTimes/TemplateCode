...
inline string tobinary(int x) {
	string newstring;
	while (x) newstring += tostring(x % 2), x = x >> 1;
	int len = newstring.size(); string final_result;
	stringstream stream;
	for (int i = 0 ; i < len ; i ++) {
...
inline string tostring (int x) {
	stringstream stream; stream << x;
	string get; stream >> get;
...
inline int digit (int x) {
	stringstream stream;
	stream << x; string getx;
	stream >> getx; stream.clear();
	return getx.size();
}
void Print_set (vector<int>& result, char* cat) {
...
void Abundant_Number (int begin, int end)
{
	for (int i = begin, sum = 0; i < end; i ++) {
		auto cur = factors [i];
		for (int factor : cur) sum += factor;
		if (sum > i) result_abundant.push_back (i);
	}
...
void Evil_Number (int begin, int end) {
	for (int i = begin; i < end; i ++) {
		int get = i, cut = 0;
		while (get) get -= (get & (-get)), cut ++;
		if (!(cut & 1)) result_evil.push_back(i);
	}
	Print_set (result_evil, "Evil");
}
void Perfect_Number (int begin, int end) {
	for (int i = begin; i < end; i ++) {
		int sum = 0; auto cur = factors[i];
		for (int factor : cur) sum += factor;
		if (sum == i) result_perfect.push_back(i);
	}
...
void Happy_Number (int begin, int end) {
	...
	for (int i = begin; i < end; i ++) {
		...
		int cur = i, flag = -1;
		while (true) {
			if (hash[cur] == -1) {flag = -1; break;}
			if (hash[cur] == 1) {flag = 1; break;} st.push(cur);
			stream << cur; string get; stream >> get;
			stream.clear(); int sum = 0; for (char ch : get)
				sum += (ch - '0') * (ch - '0'); cur = sum;
		}
		while (!st.empty()) hash[st.top()] = flag, st.pop();
		if (flag == 1)result_happy.push_back(i);
	}
...
void Primeval_Number (int begin, int end) {
	...
	set <int> prime_get; int cut = 1;
	for (int i = 3; i < end; i ++) {
		string get = tostring(i);
		int len = get.size(), tot = (1 << len);
		for (int j = 1; j < tot; j ++) {
			string index = tobinary(j); stringstream stream;
			stream << setw(len) << setfill('0') << index;
			index = stream.str(); stream.clear(); string getnew;
			for (int k = 0 ; k < index.size(); k ++)
				if (index[k] == '1')getnew += get[k];
			sort (getnew.begin(), getnew.end());
			do {
				int getnewnum = toint(getnew);
				if (!isnotprime[getnewnum]) prime_get.insert(getnewnum);
			} while (next_permutation(getnew.begin(), getnew.end()));
		}
		int cur = prime_get.size(); prime_get.clear();
		if (cur > cut) cut = cur, result_primeval.push_back(i);
	}
